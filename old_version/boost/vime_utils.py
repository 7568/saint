...
def perf_metric(metric, y_test, y_test_hat):
    ...
    # Accuracy metric
    if metric == 'acc':
        result = accuracy_score(y_test, np.argmax(y_test_hat, axis=1))
    # AUROC metric
    elif metric == 'auc':
...
def perf_metric_lgb(metric, y_test, y_test_hat):
    """Evaluate performance.
    ...
    # Accuracy metric
    if metric == 'acc':
        result = accuracy_score(y_test, np.array(y_test_hat > 0.5).astype(int))
    # AUROC metric
    elif metric == 'auc':
