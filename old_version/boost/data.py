...
def data_prep(dataset, seed, mask_det=None, datasplit=[.65, .15, .2]):
    np.random.seed(seed)
    if dataset in ['1995_income', 'bank_marketing', 'qsar_bio', 'online_shoppers', 'blastchar', 'htru2', 'shrutime',
                   'spambase', 'arcene', 'creditcard', 'arrhythmia', 'forest', 'kdd99']:
        ...
        for col in train.columns[train.dtypes == 'int64']:
            train.fillna(train.loc[train_indices, col].mean(), inplace=True)
            unique_data = np.unique(train.loc[:, col].to_numpy())
            if len(unique_data)<10:
                categorical_columns.append(col)
                categorical_dims[col] = len(unique_data)
        ...
        if self.is_pretraining:
            if self.tag is not None:
                return np.concatenate((self.X1[idx], self.y[idx])), self.X2[idx], np.concatenate(
                    (self.X1_mask[idx], self.y_mask[idx])), self.X2_mask[idx], self.tag[idx]
            else:
                return np.concatenate((self.X1[idx], self.y[idx])), self.X2[idx], np.concatenate(
...
def vision_data_prep(dataset, seed, mask_det=None, datasplit=[0.8, 0.2]):
    ...
    if dataset == 'mnist':
        ...
        X_train['data'], y_train['data'] = torch.flatten(temp.data[train_loc, :, :].long(), start_dim=1,
                                                         end_dim=-1).numpy(), temp.targets[train_loc].numpy()
        X_valid['data'], y_valid['data'] = torch.flatten(temp.data[valid_loc, :, :].long(), start_dim=1,
                                                         end_dim=-1).numpy(), temp.targets[valid_loc].numpy()
        test_set = datasets.MNIST(root=data_dir, train=False,
                                  download=True)
        X_test['data'], y_test['data'] = torch.flatten(test_set.data.long(), start_dim=1,
                                                       end_dim=-1).numpy(), test_set.targets.numpy()

    else:
        raise 'Dataset not found'

    cat_dims, cat_idxs, con_idxs = np.repeat(256, X_train['data'].shape[-1]), np.arange(
        X_train['data'].shape[-1]), np.array([])
    if mask_det is not None:
