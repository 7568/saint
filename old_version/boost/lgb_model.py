...
import argparse
from vime_utils import perf_metric_lgb
...
def lgb_model(x_train, y_train, x_vaidation, y_validation, x_test):
    """XGBoost.
    ...
    # - reg_alpha = 0.5,
    # - reg_lambda = 0.5,
    params = {'objective': 'binary', 'learning_rate': 0.01, 'max_depth': -1, 'num_leaves': 2 ** 8, 'lambda_l1': 0.5,
             'lambda_l2': 0.5, 'feature_fraction': 0.75, 'bagging_fraction': 0.75, 'bagging_freq ': 1, 'metric': 'auc'}

    train_data = lgb.Dataset(x_train, label=y_train.reshape(-1))
    num_round = 50000
    validation_data = lgb.Dataset(x_vaidation, label=y_validation.reshape(-1))
...
if __name__ == '__main__':
    ...
    y_test_hat = lgb_model(X_TRAIN['data'], Y_TRAIN['data'], X_VALID['data'], Y_VALID['data'], X_TEST['data'])
    performance = perf_metric_lgb(metric, np.array(Y_TEST['data']).reshape(-1), y_test_hat)
